...
Reads live from Google Sheets (five sheets):
  ...
  MONTHLY_TARGET        → new product names + KPIs (target/sales/deficit)
                          col A=bag, col C=target, col D=sales, col E=deficit, col M=flag
...
# Edit bags_selection_config.json (shops: "ALL" or a list; dates YYYY-MM-DD).
CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bags_selection_config.json")
...
def fetch_new_products_data():
    ...
    # col D (idx 3) = SALES
    # col E (idx 4) = DEFICIT
    # col M (idx 12) = BAGS SELECTION flag
    ...
    for row in mt_rows[1:]:
        ...
        if len(row) < 13 or not is_checked(row[12]):
            continue
    ...
    print(f"  Bags selection found    : {len(new_product_names)}")
...
WEEKLY_POSTS_HISTORY = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                    "bags_selection_weekly_history.json")
...
html_path = os.path.join(BASE_DIR, "bags_selection.html")
...
print("bags_selection.html updated.")
