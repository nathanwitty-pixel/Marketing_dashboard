...
## Bags not on offer (`#smc-noffer-panel`, `SMC.bagsNotOnOffer`)

Bags **selling this month but on no offer** — not a running-combo component, Deal of the
Week, or Power Deal. `_bags_not_on_offer` sums standalone bag sales (`BAG_SALES_SQL`),
resolves each Odoo product to a catalogue bag (`bag_original_prices.json` keys,
longest-prefix), and marks a bag on-offer if it matches (equal or word-prefix, aliases
applied: Cairo Backpack→Cairo BP, Briefcase→Brief Case, …) any running/deal bag. The panel
shows the count/units/revenue not on offer + the on-offer coverage %, and lists the
not-on-offer bags by units (candidates to put on an offer).
